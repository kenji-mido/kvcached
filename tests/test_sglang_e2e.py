...
"""
SGLang + kvcached E2E elastic memory benchmark.

Usage (inside SGLang ROCm Docker container):
  pip install -e /kvcached --no-build-isolation
  python /kvcached/tests/test_sglang_e2e.py             # kvcached enabled
  python /kvcached/tests/test_sglang_e2e.py --baseline   # vanilla SGLang
"""
...
class MemoryPoller:
    ...
    def print_timeline(self, title, max_rows=25, bar_width=30):
        ...
        has_kv = any(s[3] > 0 for s in self.samples)
...
def generate_batch(engine, prompts, max_tokens):
    """Generate a batch of prompts using SGLang Engine (true batch)."""
    engine.generate(prompts, {"max_new_tokens": max_tokens, "temperature": 0.8})
...
def main():
    mode = "BASELINE (vanilla SGLang)" if BASELINE else "kvcached ENABLED"
    print("=" * 80)
    print(f"  SGLang + kvcached Elastic Memory Benchmark  [{mode}]")
    print("=" * 80)
    if not BASELINE:
        ...
        print("  kvcached is DISABLED — SGLang will pre-allocate all KV cache memory.")
    ...
    for label, n_prompts, max_tokens, prompts in WORKLOADS:
        ...
        if label.startswith("Phase 4"):
            if not BASELINE:
                ...
                print("  SGLang pre-allocated all KV memory — nothing changes.")

        poller = MemoryPoller(interval=0.5 if prompts is None else 0.1)
        poller.start()

        if prompts is not None:
            generate_batch(engine, prompts, max_tokens)
            time.sleep(2)
        else:
            time.sleep(4)
    ...
    if all_pass:
        if BASELINE:
            print("  ALL CHECKS PASSED — Confirmed: vanilla SGLang is NOT elastic.")
        else:
